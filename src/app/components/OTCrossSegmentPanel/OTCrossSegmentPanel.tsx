...
import { ElevadrReport, OTcrossSegmentLinesPanel, ServiceConnectionDetail } from '../../types/Report';
...
const OTCrossSegmentPanel: React.FC<OTCrossSegmentPanelProps> = ({ data, reportId }) => {
  const drilldown = useDrilldown(({ src_subnet, dst_subnet }: { src_subnet: string; dst_subnet: string }) =>
    fetchCrossSegmentDrilldown(reportId, src_subnet, dst_subnet)
  );
  const payload: OTcrossSegmentLinesPanel =
    data ?? {
      lines: [],
      subnet_pair_counts: [],
      dst_subnet_counts: [],
  ...
  const handleSubnetPairClick = async (row: { src_subnet: string; dst_subnet: string }) => {
    ...
    >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'flex', gap: '16px', alignItems: 'baseline', flexWrap: 'wrap' }}>
            ...
            <div style={{ color: '#6b6b6b' }}>cross-segment communication example(s)</div>
          </div>
              ...
              columns={[
                {
                  ...
                  sortable: true,
                  clickable: true,
                  onClick: (_value, row) => handleSubnetPairClick(row as { src_subnet: string; dst_subnet: string }),
                },
                {
                  key: 'dst_subnet',
          ...
          <section style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <SortableTable
              columns={[
                { key: 'dst_subnet', label: 'Destination Subnet', sortable: true },
          ...
          <section style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <SortableTable
              columns={[
                ...
                { key: 'count', label: 'Cross-Segment Flow Count', sortable: true, align: 'right' },
              ] satisfies Column[]}
              ...
              columns={[
                {
                  ...
                  label: 'Source IP',
                  sortable: true,
                  render: (value: unknown) => formatIpForDisplay(typeof value === 'string' ? value : null),
                },
                {
                  key: 'dst_endpoint.ip',
                  label: 'Destination',
                  ...
                  render: (_value: unknown, row: Record<string, unknown>) => {
                    const destinationIp = formatIpForDisplay(
                      typeof row['dst_endpoint.ip'] === 'string' ? row['dst_endpoint.ip'] : null
                    );
                    const destinationPort = row['dst_endpoint.port'] ?? 'Unknown';
      ...
      <DetailModal
        ...
        title={drilldown.selectedKey ? `Cross-Segment Details: ${drilldown.selectedKey.src_subnet} → ${drilldown.selectedKey.dst_subnet}` : 'Cross-Segment Details'}
        onClose={closeModal}
      >
        {drilldown.isLoading && <p>Loading cross-segment connection details...</p>}
        {drilldown.error && <p>{drilldown.error}</p>}
        {!drilldown.isLoading && !drilldown.error && (
          <SortableTable
            columns={[
              { key: 'src_endpoint.ip', label: 'Source IP', sortable: true },
              { key: 'src_endpoint.subnet', label: 'Source Subnet', sortable: true },
              { key: 'src_endpoint.port', label: 'Src Port', sortable: true, align: 'right' },
              { key: 'dst_endpoint.ip', label: 'Destination IP', sortable: true },
              { key: 'dst_endpoint.subnet', label: 'Destination Subnet', sortable: true },
