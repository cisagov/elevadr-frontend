...
const GenericDevicePanel: React.FC<GenericDevicePanelProps> = ({ data = [], title, deviceType }) => {
  const columns: Column[] = [
    ...
    },
    {
      ...
      sortable: true,
      render: (services: string[] | null) => (services ? services.join(', ') : 'N/A'),
    },
  ];
    ...
    <Panel
      ...
      title={
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span>{title}</span>
          <InfoTooltip text={`A list of all identified ${deviceType} devices on the network.`} />
        </div>
      }
      isEmpty={isEmpty}
    >
