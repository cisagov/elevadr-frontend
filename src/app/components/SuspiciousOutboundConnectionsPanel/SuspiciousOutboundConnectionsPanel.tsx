...
import { ElevadrReport, ServiceConnectionDetail, SuspiciousOutboundConnection } from '../../types/Report';
...
const SuspiciousOutboundConnectionsPanel: React.FC<SuspiciousOutboundConnectionsPanelProps> = ({ data, reportId }) => {
  const drilldown = useDrilldown((row: SuspiciousOutboundRow) =>
    fetchSuspiciousOutboundDrilldown(reportId, row.srcIp, row.dstIp, row.port, row.service)
  );
  ...
  return (
    ...
    >
      ...
      <DetailModal
        ...
        title={drilldown.selectedKey ? `Suspicious Outbound Details: ${drilldown.selectedKey.srcIp} → ${drilldown.selectedKey.dstIp}:${drilldown.selectedKey.port}` : 'Suspicious Outbound Details'}
        onClose={closeModal}
      >
        {drilldown.isLoading && <p>Loading suspicious outbound connection details...</p>}
