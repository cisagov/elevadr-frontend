...
import { fetchFilteredConnections, fetchFilteredDevices } from '../../services/drilldownService';
import DrilldownTable, { connectionDetailColumns } from '../DrilldownTable/DrilldownTable';
...
const DevicesPanel: React.FC<DevicesPanelProps> = ({ otDevices, itDevices, edgeDevices, reportId }) => {
  const deviceDrilldown = usePivotDrilldown((filters: Record<string, string | number | boolean | null | undefined>) =>
    fetchFilteredDevices(reportId, filters)
  );
  const connectionDrilldown = usePivotDrilldown((filters: Record<string, string | number | boolean | null | undefined>) =>
  ...
  const isEmpty = otDevices.length === 0 && itDevices.length === 0 && edgeDevices.length === 0;
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        <section id="ot-devices-panel" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <SortableTable columns={deviceColumns} data={otDevices} filterable={true} filterPlaceholder="Search OT devices..." emptyMessage="No Results" />
        </section>

        <section id="it-devices-panel" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <h3 className="section-subtitle">
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <SortableTable columns={deviceColumns} data={itDevices} filterable={true} filterPlaceholder="Search IT devices..." emptyMessage="No Results" />
        </section>

        <section id="edge-devices-panel" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <h3 className="section-subtitle">
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <SortableTable columns={deviceColumns} data={edgeDevices} filterable={true} filterPlaceholder="Search Edge devices..." emptyMessage="No Results" />
        </section>
      </div>

      <DetailModal isOpen={Boolean(deviceDrilldown.selectedFilters)} title="Filtered Devices" onClose={deviceDrilldown.close}>
          ...
          <>
            ...
            <DrilldownTable
              columns={buildDeviceColumns(
                ...
                (subnet) => void connectionDrilldown.open({ subnet, limit: 500 }),
                (service) => void connectionDrilldown.open({ service_name: service, limit: 500 }),
                (service) => void connectionDrilldown.open({ service_name: service, limit: 500 }),
              )}
      ...
      <DetailModal isOpen={Boolean(connectionDrilldown.selectedFilters)} title="Filtered Connections" onClose={connectionDrilldown.close}>
        {connectionDrilldown.isLoading && <p>Loading matching connections...</p>}
